...
Otherwise it is a ticket. One run that crashed, a bug that will be fixed next month, or a problem that only appears on one person's branch belongs in the issue tracker of whichever repository is at fault. Search https://github.com/AWI-ESM/project_management before you open one, since several hundred past and present issues are collected there and most problems with this model have been described already. That repository is private, so ask one of the AWI staff named in :doc:`before_you_start` if you cannot reach it.
