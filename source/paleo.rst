...
- Do not reuse the OASIS ``rmp_`` weight files from the old resolution. They are grid pair specific and have to be regenerated; for large grids see :doc:`how_to` for the offline parallel weight generation.
