...
- **The suffix names a configuration, not a patch level.** ``AWI-ESM3-v3.5.0-cc`` is not a bugfix release of ``AWI-ESM3-v3.5.0``, and ``-is`` is not a later version than ``-cc``. They are three configurations that share an ocean and an atmosphere, gated in esm_tools by feature switch rather than by version, so read the suffix as "with the carbon cycle" and "with the ice sheet". This is the same convention as ``EC-Earth3-CC`` and ``EC-Earth3-Veg``, and as AWI's own CMIP6 ``AWI-CM-1-1-MR``, where the resolution suffix sits in the same position.
